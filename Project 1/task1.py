    ...
    for r in range(1, len(padded_img) - 1):
        for c in range(1, len(padded_img[0]) - 1):
            for _r, _c in neighbors:
                img_conv[r - 1][c - 1] += padded_img[r + _r][c + _c] * flipped_kernel[1 + _r][1 + _c]
...
def normalize(img):
    ...
    """
    # TODO: implement this function.
    minPixel = min([min(row) for row in img])
    maxPixel = max([max(row) for row in img])
    if isinstance(img, list):
        for r in range(len(img)):
            for c in range(len(img[0])):
                img[r][c] = (img[r][c] - minPixel) * 255 / (maxPixel - minPixel)

    elif isinstance(img, np.ndarray):
        img = np.divide((img - minPixel), (maxPixel - minPixel))
    #raise NotImplementedError
    return img
...
def edge_magnitude(edge_x, edge_y):
    ...
    """
    # TODO: implement this function.
    # edge_mag = [[0 for i in range(len(edge_x[0]))] for j in range(len(edge_x))]
    # for r in range(len(edge_mag)):
    #     for c in range(len(edge_mag[0])):
    #         edge_mag[r][c] = math.sqrt(edge_x[r][c] ** 2 + edge_y[r][c] ** 2)

    edge_mag = np.sqrt(np.multiply(edge_x, edge_x) + np.multiply(edge_y, edge_y))
    #raise NotImplementedError
